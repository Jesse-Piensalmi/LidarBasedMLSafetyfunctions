...
if PYSIM:
    ...
    from control_lib.SVC_h import OnlineSVMModel
...
class Controller():
    def __init__(self): # INITIALIZE CONTROLLER
        ...
        self.svm = [OnlineSVMModel(min_d_sample=SceneSetup.min_d_sample,
                                  iter_mem=SceneSetup.iter_mem,
                                  grid_size_plot=SceneSetup.grid_size_plot,
                                  dh_dt=SceneSetup.dh_dt, envelope=SceneSetup.envelope) for _ in range(SceneSetup.robot_num)]
    ...
    def compute_control(self, feedback, computed_control): # INITIALIZE CONTROLLER
        ...
        #for i in [2]:
            ...
            for j in range(360):
                if ((sensing_data[j] < SceneSetup.sense_dist and j%1 ==0) or (j%1 == 0 and SceneSetup.envelope)): 
                    #get the location of the detected obstacle edges 
                    #edge_data_X=np.reshape(sensor_pos_data[j,0:2], (1,2))
                    #edge_data_Y=np.array([[-2]])
                    #self.svm[i].set_new_data(edge_data_X, sense_dist=SceneSetup.sense_dist, rob_pos= current_q_center)
                    edge_data_X=np.reshape(sensor_pos_data[j,0:2], (1,2))
                    #edge_data_Y=np.array([[-2]])
                    self.svm[i].set_new_data(edge_data_X, dist=sensing_data[j], sense_dist=SceneSetup.sense_dist, rob_pos= current_q_center)
                    if(sensing_data[j] < SceneSetup.sense_dist and len(self.svm[i].data_X) > 5):
