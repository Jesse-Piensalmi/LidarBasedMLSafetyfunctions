...
def scenario_pkl_plot():
    ...
    for robot_id in range(SceneSetup.robot_num):
        ...
        # WIDHI: 
        # 1. I turned off the rcParams.update with FS to let it use the default value
        #   and turned on the latex setup (rcParams['text.usetex'])
        #   This lets the text size seems normal-ish and reduce overlapping
        # 2. I swapped the order of h_gp_ and min_lidar
        #   This way when you cut the min_lidar for the journal, you can keep the x-axis label of time
        #   I also added the dashed red line for the robot's size. 
        #   Although in my opinion, it should be the safe distance that you set in your CBF controller.
        #   You can adjust it later as you need it.
        # 3. I combined u_x and u_y into one subplot. 
        #   I think for the journal, just the plot of ||u|| should be enough.
        ...
        ax[1].plot(__stored_data['time'][:__end_idx], [0.09 for _ in range(__end_idx)], linestyle='dashed', color='r', label='robot\'s size')
        # label
        ...
        ax[0].set_ylabel(r'$u$[m/s]', fontsize=14 )
        ax[1].set_xlabel(r'$t$ [s]', fontsize=14)
...
def exp_video_pkl_plot():
    ...
    def_folder = '../'
    videoloc = def_folder + 'ZED_record_20240919_204232.avi'
    ...
    time_snap = [0, 28, 58, 88]  # in seconds
    ...
    if videoloc is not None:
        ...
        while True:
            ...
            if ret:
                ...
                # ------------------------------------------------------------------
                # WARNING!!!!!!
                # THIS LINE OF CODE IS ADDED AS A QUICK FIX 
                # DUE TO UNUSUAL EXPERIMENT WITH ONE ROBOT (green robot, index 2)
                poses_center[0], poses_ahead[0] = poses_center[2], poses_ahead[2]
                # color manipulation. should be above, but put here so it is easier to remove
                h = robot_color[2].lstrip('#')
                r, g, b = tuple(int(h[i:i+2], 16) for i in (0, 2, 4))
                bgr_color[0] = (b, g, r)
                # ON NORMAL CONDITION REMOVE OR COMMENT THE ABOVE LINES
                # ------------------------------------------------------------------
