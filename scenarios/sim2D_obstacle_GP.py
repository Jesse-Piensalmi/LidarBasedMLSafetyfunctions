...
class SceneSetup(): 
    ...
    obstacle = [#0.8*np.array([[-1,0.4 ,0], [-1,1,0], [0.2,1 ,0], [0.2, -1 ,0], [-1,-1 ,0],[-1,-0.4 ,0],[-0.7,-0.4 ,0], [-0.7,-0.7 ,0], [0,-0.7 ,0], [0,0.7 ,0], [-0.7,0.7 ,0], [-0.7,0.4 ,0], [-1,0.4 ,0]])]
            0.4*np.array([[ 1., 0., 0], [ 0.5, np.sqrt(3)/2, 0], [-0.5, np.sqrt(3)/2, 0], [-1., 0., 0], [-0.5, -np.sqrt(3)/2, 0], [ 0.5, -np.sqrt(3)/2, 0], [ 1.,  0., 0]])-np.array([0.5,0,0])]
...
class Controller():
    def __init__(self): # INITIALIZE CONTROLLER
        ...
        self.gp = [GP(SceneSetup.hypers_gp,SceneSetup.exp_decay_rate,SceneSetup.min_d_sample, SceneSetup.iter_mem, SceneSetup.grid_size_plot, SceneSetup.dh_dt) for _ in range(SceneSetup.robot_num)]
    ...
    def compute_control(self, feedback, computed_control): # INITIALIZE CONTROLLER
        ...
        #for i in [2]:
            ...
            if self.gp[i].N == 0:
                ...
                gp_G=np.append(gp_G,np.array([[0]]), axis=1)
                if true_gp_h<0:
...
class SimSetup():
    ...
    sim_defname = '/home/jesse/LiDARBasedCBFRobo666/lidar_gp_cbf/control_lib/SimData/SimData/Hexagone_test'
