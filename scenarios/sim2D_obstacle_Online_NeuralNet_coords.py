...
if PYSIM:
    ...
    from control_lib.NeuranNetLSTM_h import SafetyNet
...
class Controller():
    ...
    def compute_control(self, feedback, computed_control): # INITIALIZE CONTROLLER
        ...
        #for i in [2]:
            ...
            #------------------------------------------------------------------
            #Update NeuralNet
            #update New Neural iteration counter and forget old data
            self.svm[i].new_iter()
            learn = False
            #Sample detected edges
            #check if any obstacle is detected and collect with sampling distance min_dis constraint
            for j in range(360):
                if (sensing_data[j] < SceneSetup.sense_dist and j%10==0) or (j%10==0 and SceneSetup.envelope==1): 
                    ...
                    if len(self.svm[i].data_X)>9 and sensing_data[j] < SceneSetup.sense_dist:
                        learn = True
            if learn:
